...
if (!function_exists('select')) {
    function select($query)
    {
        ...
        return $rows; // ✅ penting: kembalikan array, bukan result object
    }
}
...
function create_mahasiswa($post)
{
    ...
    // Ambil foto dari $_FILES
    $foto = upload_file($_FILES['foto']);  

    if (!$foto) {
        return false; // kalau gagal upload, hentikan
    }

    // Query dengan sebutkan nama kolom
    $query = "INSERT INTO mahasiswa (nama, prodi, jk, telepon, alamat, email, foto)
              VALUES ('$nama', '$prodi', '$jk', '$telepon', '$alamat', '$email', '$foto')";

    mysqli_query($db, $query) or die("MySQL Error: " . mysqli_error($db));
...
// funsi menghapus data mahasiswa
...
function create_akun($post)
{
    ...
    //Tambahkan pengecekan error jika masih troubleshooting
    if (mysqli_error($db)) {
